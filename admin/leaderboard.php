    ...
    <div class="container">
        ...
        <table>
            ...
            <?php 
                ...
                $sql = "select @a:=@a+1 as rank, u.name as name, u.id as iduser, count(sb.c_id) as solved, sum(ch.score) as score from (SELECT @a:= 0) AS a, users as u, challenges as ch, scoreboard as sb where sb.c_id = ch.id and sb.user_id = u.id group by sb.user_id order by score desc";
                ...
                if ($count > 0) {
                    while ($row = mysqli_fetch_array($result, MYSQLI_ASSOC)) {
                        ...
                        $idusss = $row['iduser'];
                        $allhint = mysqli_query($conn, "SELECT  *, sum(challenges.hintpoint) as totalhint FROM `hintcollect` JOIN challenges WHERE challenges.id = hintcollect.idchall AND hintcollect.iduser = $idusss;");
                        $rowhint = mysqli_fetch_array($allhint, MYSQLI_ASSOC);
                        
                        echo "<tr class='content'>";
                        echo "<td>".$i++."</td>";
                        echo "<td>".$row["name"]."</td>";
                        echo "<td>".($row["score"] - $rowhint['totalhint'])."</td>";
